...
public class DeduplicationExercise {
    public static void main(final String[] args) {
        ...
        System.out.println(doDeduplicateWithEquals(integerList));
        //System.out.println(doDeduplicateWithHashCode(integerList));
    }

    private static List doDeduplicateWithEquals(final List<Integer> integerList){
    ...
    private static List doDeduplicateWithHashCode(final List<Integer> integerList){
        for (int i =0; i < integerList.size(); i++) {
            for (int j = i + 1; j < integerList.size(); j++) {
                if (integerList.get(i).hashCode() == integerList.get(j).hashCode()){
                    integerList.remove(j);
                }
            }
        }
